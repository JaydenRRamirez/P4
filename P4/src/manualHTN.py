...
pyhop.declare_operators(op_punch_for_wood, op_craft_wooden_axe_at_bench, 
                        op_wooden_axe_for_wood, op_craft_bench, 
                        op_craft_plank, op_craft_stick)
...
def produce(state, ID, item):
	if item == 'wood': 
		...
		if state.made_wooden_axe[ID] is True: return False
...
def craft_wooden_axe_at_bench(state, ID):
	return [('have_enough', ID, 'bench', 1), ('have_enough', ID, 'stick', 2), 
	        ('have_enough', ID, 'plank', 3), ('op_craft_wooden_axe_at_bench', ID)]
